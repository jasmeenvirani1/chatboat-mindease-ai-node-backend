...
const EMOJI_REGEX = /[\p{Extended_Pictographic}\u{FE0F}\u{FE0E}\u{1F3FB}-\u{1F3FF}\u{1F9B0}-\u{1F9B3}]/gu;

// Expanded Emoticons Regex
// Matches: :) :( :D :P ;) :O <3 xD o_o -_- (y) (n) etc.
const EMOTICON_REGEX = /[:;][-]?[()DPpSsoO/\\]|[(][yYnN][)]|<3|[oO]_[oO]|[-]_[-]|x[D]/g;
...
function applyPurpleDotBranding(text) {
  ...
  if (finalDots > 2) {
    ...
    cleanedText = parts[0] + "[PURPLE_DOT]" + parts[1] + "[PURPLE_DOT]" + parts.slice(2).join(" ");
    cleanedText = cleanedText.replace(/\s\s+/g, " ").trim();
  }
