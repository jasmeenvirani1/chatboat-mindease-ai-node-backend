...
// ─────────────────────────────────────────────────────────────────────────────
// SPANISH TONE MATRICES — Three variants
// spanishTone: "neutral" | "spain" | "mexico"
// All instructions in English; tone examples in Spanish to show target output style.
// ─────────────────────────────────────────────────────────────────────────────

const ES_TONE_NEUTRAL = `
...
- Elegant and serene — precise vocabulary, no emotional excess
- Slightly formal — respectful distance without coldness
- Sober and clear — direct, refined Iberian clarity
- No diminutives, no regionalisms, no excessive warmth
- No mystical tone, no spiritual destiny language

NEVER use: sentimental clichés, exclamation marks, self-help language.
ALWAYS sound like: a composed, intellectual Spanish speaker who uses language with elegance and calm.

Tone examples (this is the style of Spanish your output should match):
- "Hoy tu energía se expresa con una claridad serena."
- "Hay una sobriedad en tu forma de avanzar."
- "Lo esencial se revela cuando mantienes la mente despejada."
`.trim();

const ES_TONE_MEXICO = `
...
function getToneMatrix(spanishTone) {
  if (spanishTone === "spain") return ES_TONE_SPAIN;
  if (spanishTone === "mexico") return ES_TONE_MEXICO;
  return ES_TONE_NEUTRAL;
}

// ─────────────────────────────────────────────────────────────────────────────
// SPANISH SIGNS PACK — 12 signs with psychological profile
// ─────────────────────────────────────────────────────────────────────────────
const ES_SIGNS = {
  aries: {
...
const ES_PLANETS = {
  ...
  saturn: "lessons, discipline, boundaries — where you are being asked to grow up",
...
// ─────────────────────────────────────────────────────────────────────────────
// PROMPT BUILDERS — 8 subcategories
// All instructions in English. AI responds in Spanish per tone rules.
// Each accepts { userMessage, dbPrompt, langName, birthChart, birthChartB?, toneMatrix }
// ─────────────────────────────────────────────────────────────────────────────
...
YOUR FOCUS: Western Zodiac Signs — psychology-based readings.
...
function buildEnergyMatchPrompt({
  ...
  return `You are Astria Spanish — a modern Western astrology relationship dynamics guide for the Spanish lane.
...
YOUR FOCUS: Life Graph — life cycles and personal rhythms.
...
// ─────────────────────────────────────────────────────────────────────────────
function buildCategoryFallbackPrompt({ dbPrompt, langName, birthChart, toneMatrix }) {
  const chartBlock = formatChartBlock(birthChart, "full");
...
const SUBCATEGORY_BUILDERS = [
  {
    keywords: ["big 3", "big3", "sun", "moon", "rising", "sol", "luna", "ascendente"],
  ...
  { keywords: ["daily", "flow", "transit", "flujo", "diario"], builder: buildDailyFlowPrompt },
  { keywords: ["life graph", "life", "graph", "ciclo", "ritmo"], builder: buildLifeGraphPrompt },
  {
...
function buildAstriaSpanishContext({
  ...
  const params = { userMessage, dbPrompt, langName, birthChart, birthChartB, toneMatrix };
  ...
  return buildCategoryFallbackPrompt({ dbPrompt, langName, birthChart, toneMatrix });
}
