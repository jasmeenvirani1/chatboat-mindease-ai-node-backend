...
const { generateSajuReading, getSajuReadingById } = require("../services/sajuService.js");
...
const scan = async (req, res) => {
  try {
    ...
    const subCategoryId = req.body?.subCategoryId || req.body?.categoryId || null;
    ...
    const result = await generateSajuReading({ ...sanitized, subCategoryId }, userId, lang);
    return res.status(200).json({ success: true, ...result });
  } catch (err) {
    console.error("[sajuController] scan error:", err?.message || err);
    return res.status(500).json({ success: false, message: "Failed to generate Saju reading. Please try again." });
  }
};
